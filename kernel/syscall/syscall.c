 ...
 */
static i64 sys_cap_create_atom(u64 atom, u64 rights, u64 expiry_ticks, u64 quota, u64 scope_hash) {
    if (atom >= ATOM_MAX)
        return (i64)ERR_INVAL;
 ...
 */
static i64
sys_cap_grant_to_subject(u64 subject, u64 atom, u64 rights, u64 expiry_ticks, u64 quota) {
    if (subject == 0)
        return (i64)ERR_NOENT;
    if (atom >= ATOM_MAX)
        return (i64)ERR_INVAL;
...
static i64 sys_set_time(u64 user_t_ptr) {
    ...
    if (t.year < 1970 || t.month < 1 || t.month > 12 || t.day < 1 || t.day > 31 || t.hour > 23 ||

...
### 第十一轮补充 3（2026-08-27）：键盘输入跟随焦点窗口

| 项 | 结果 |
|---|---|
| **问题**：合成器把键盘事件广播给所有窗口，demo 收到 KEY 无条件回显到 Keys 窗口——点击其他窗口聚焦后键盘仍进 Keys | ✅ |
| **修复**：`gui_event_t` 增加 `win` 字段——合成器 push KEY 时填 `s_focus_id`（焦点窗口），MOUSEMOVE/BUTTON 填指针命中窗口；demo 的 KEY 处理先检查 `e->win == w1`（Keys 窗口）才回显；`q`/Esc 保持全局退出 | ✅ |
| **验证**：无焦点键入不回显 → 点击 Keys 聚焦后回显（像素验证）→ 点击 Canvas 聚焦后不回显 → q 任意焦点退出 | ✅ |

### 第十一轮补充 4（2026-08-27）：重叠窗口下层穿透修复

| 项 | 结果 |
|---|---|
| **问题**：脏区合成时窗口**整窗画边框+标题栏**（不裁剪到脏区）——上层窗口覆盖下层窗口的边框/标题区域时，下层边框残影穿到上层（"两个窗口同时显示"） | ✅ |
| **修复**：composite 窗口绘制全部裁剪——边框四条边逐边与脏区求交（hline/vline clip）；标题栏 fill 裁剪、文本仅当脏区触及标题栏时整行重画（无害）；内容 blit 保持裁剪 | ✅ |
| **验证**：w2 拖到 w1 上——重叠区只显示 w2 内容（16,16,16），w1 黑色不穿透；鼠标多次移动触发重绘后仍正确；w1 边框不透过 w2 标题栏 | ✅ |

### 第十一轮补充 5（2026-08-27）：悬停下层标题栏穿透修复

| 项 | 结果 |
|---|---|
| **问题**：悬停在下层窗口标题栏时，下层标题文本仍显示到上层窗口（仅标题栏）——两个根因：(a) draw_titlebar 画完整 16px 字符，上层内容只按 dirty 裁剪覆盖（y 方向，字符下半截残留）；(b) 部分在 dirty 内的字符画满 8px 宽（x 方向，字符尾巴残留） | ✅ |
| **修复**：draw_titlebar 改为逐字符、**逐行 y clip**、**逐列 x clip** 绘制（gui_font + gui_pixel，只画与 dirty 相交的行/列）——下层标题任何像素都不会画到上层窗口的区域 | ✅ |
| **验证**：w2 盖住 w1 标题后，多次跨边界悬停——w2 内容区 **0 个新亮像素**（此前 22/451 个穿透）；w1 可见标题完整；diff 对比悬停前后确认 | ✅ |

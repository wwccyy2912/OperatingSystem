 ...
 */
static error_t blk_negotiate(u16 io_base, u64 vq_phys,
                             u16 *out_num, u64 *out_capacity)
{
...
static error_t blk_lazy_init(void)
{
     ...
     * authoritative table index (what capability obj_ids name). */
        ...
        memset((void *)(s_vq_phys + KERNEL_VIRT_BASE), 0,
           VQ_PAGES(VQ_MAX_NUM) * PAGE_SIZE);
        ...
        serial_printf("blk: virtio-blk pci[%d] io=0x%x vq=%u slots=%u sectors=%u\n",
                                    idx, (unsigned)io_base, (unsigned)num, (unsigned)num,
                                    (u64)capacity);
        return OK;
}
 ...
 */
static error_t blk_io_one(u64 disk, bool is_write, u64 sector,
                                                    u32 nsectors, void *user_buf)
{
        if (disk != (u64)s_device_index)
                return ERR_INVAL;
        ...
        *(u64 *)(bounce + 8) = sector;
        ...
        desc[1].len  = data_len;
        desc[1].flags = is_write ? VIRTQ_DESC_F_NEXT
     ...
     * used.idx.  Verify it is VIRTIO_BLK_S_OK (0). */
        ...
        bool ok = (used->ring[(used_idx - 1) % s_queue_num].id == 0) &&
...
static error_t blk_io_loop(u64 disk, bool is_write, u64 lba, u64 count,
                           void *user_buf)
{
        u64 remaining = count;
        u64 cur = lba;
        ...
        while (remaining > 0) {
                u32 n = (remaining > BLK_MAX_SECTORS)
                                        ? BLK_MAX_SECTORS
                                        : (u32)remaining;
...
static bool proc_has_pci_dev_cap(u64 obj_id)
{
        ...
        for (u32 i = 0; i < MAX_CAPS; i++) {
                ...
                if (e->type == CAP_TYPE_PCI_DEV && e->obj_id == obj_id &&
                        (e->rights & (RIGHT_READ | RIGHT_WRITE)) ==

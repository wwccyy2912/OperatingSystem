...
static struct { char name[64]; port_t port; } s_port_registry[PORT_REGISTRY_SIZE];
...
static void pending_free(ipc_pending_msg_t *m) {
    if (!m) return;
    m->in_use = false; m->sender_tid = -1;
    m->next = s_free_list; s_free_list = m;
}

static port_entry_t *port_lookup(port_t p) {
    if (p == PORT_NULL || p > MAX_PORTS) return NULL;
...
static void deliver_to_waiter(port_entry_t *p, thread_t *recv,
                              const void *msg, u32 len, u32 token,
                              subject_id_t sender_subject)
{
    ...
    while (*pp) {
        if (*pp == recv) { *pp = recv->next; recv->next = NULL; break; }
        pp = &(*pp)->next;
    }
    wake_thread(recv);
}
...
static error_t block_with_pending(port_t port, ipc_pending_msg_t *m,
...
void ipc_port_destroy(port_t port)
{
    ...
    if (!p) { spin_unlock(&s_ipc_lock); return; }
...
error_t ipc_send(port_t port, const void *msg, u32 len)
{
    spin_lock(&s_ipc_lock);
    port_entry_t *p = port_lookup(port);
    if (!p) { spin_unlock(&s_ipc_lock); return ERR_NOENT; }
    if (len > MAX_MSG_SIZE) { spin_unlock(&s_ipc_lock); return ERR_INVAL; }
    ...
    if (recv) {
        ...
        if (!m) { spin_unlock(&s_ipc_lock); return ERR_NOMEM; }
        m->msg_len = len;
        m->sender_subject = ipc_sender_subject();
        if (msg && len > 0)
            ipc_memcpy(m->msg_data, msg, len);
        deliver_to_waiter(p, recv, m->msg_data, len, 0,
                          m->sender_subject);
        pending_free(m);
        spin_unlock(&s_ipc_lock);
        return OK;
                      ...
                      subject_id_t *sender_subject)
{
    ...
    port_entry_t *p = port_lookup(port);
    if (!p) { spin_unlock(&s_ipc_lock); return ERR_NOENT; }
    if (tok_ptr) *tok_ptr = 0;
    if (sender_subject) *sender_subject = 0;
    ...
    if (tok_ptr) *tok_ptr = rs.token;
    if (sender_subject) *sender_subject = rs.sender_subject;
    error_t err = rs.err;
    spin_unlock(&s_ipc_lock);
    return err;
}
...
error_t ipc_call(port_t port, const void *req, u32 req_len,
                 void *resp, u32 *resp_len)
{
    spin_lock(&s_ipc_lock);
    port_entry_t *p = port_lookup(port);
    if (!p) { spin_unlock(&s_ipc_lock); return ERR_NOENT; }
    if (req_len > MAX_MSG_SIZE) { spin_unlock(&s_ipc_lock); return ERR_INVAL; }
    ...
    if (recv) {
        ...
        deliver_to_waiter(p, recv, m->msg_data, req_len, token_of(m),
                          m->sender_subject);
        m->next = s_reply_wait[port - 1];
        s_reply_wait[port - 1] = m;
    } else {
...
error_t ipc_reply(u32 token, const void *msg, u32 len)
{
    ...
    if (!m) { spin_unlock(&s_ipc_lock); return ERR_NOENT; }
    if (!m->is_call || m->replied) { spin_unlock(&s_ipc_lock); return ERR_BUSY; }
    ...
    while (*pp) {
        if (*pp == m) { *pp = m->next; m->next = NULL; break; }
        pp = &(*pp)->next;
    }
...
error_t ipc_register_port(const char *name, port_t port)
{
    ...
    if (!port_lookup(port)) { spin_unlock(&s_ipc_lock); return ERR_NOENT; }

         ...
         * their handles/enums/bookmarks. */
               ...
               "volume(s)\n", v->driver_name, ret);
                for (u32 i = 0; i < MAX_VOLS; i++)
                        if (s_vols[i].mounted &&
                                strcmp(s_vols[i].driver_name, v->driver_name) == 0)
                                vfs_vol_drop(i);
        }
        return ret;
}
 ...
 * of it was granted.
 */
static int perm_check(const vfs_resource_t *res, u32 access,
                                            const char *url, u64 subject_id, u32 *granted)
{
        if (s_perm_port < 0) {
                               ...
                               vfs_bookmark_t *blob)
{
        ...
        blob->payload_len = (u32)(sizeof(*blob) -
                                                            offsetof(vfs_bookmark_t, resource));
...
{
        if (blob->magic != VFS_BOOKMARK_MAGIC ||
                blob->version != VFS_BOOKMARK_VERSION)
                return NULL;
        ...
        for (int i = 0; i < MAX_BOOKMARKS; i++) {
                ...
                if (s_vols[b->vol_index].uuid.hi != blob->resource.vol.hi ||
                        ...
                        continue;
                if (b->item_id != blob->resource.id ||
                        b->parent_id != blob->parent_id ||
                        b->access != blob->access ||
                        b->subject_id != blob->subject_id ||
                        b->created_ticks != blob->created_ticks ||
                        b->expiry_ticks != blob->expiry_ticks)
                        continue;
        ...
        for (u32 i = 0; i < MAX_VOLS; i++) {
             ...
             * count is exactly the driver-side index for this volume. */
                        ...
                        for (u32 j = 0; j < MAX_VOLS; j++)
                                if (s_vols[j].mounted &&
                                        strcmp(s_vols[j].driver_name, req->driver_name) == 0)
                                        v->drv_vol++;
                        v->mounted = 1;
...
static void do_unmount(int token, int msg_len)
{
     ...
     * gate as do_mount: exact match against the static config). */
        ...
        printf("vfs: volume '%s' unmounted (driver '%s')\n",
           s_vols[vi].mount_name, s_vols[vi].driver_name);
        vfs_vol_drop((u32)vi);
        resp->ret = 0;
...
static void do_create_dir(int token, int msg_len, u64 caller_subject)
{
     ...
     * 仅角色链约束。在驱动改动文件系统之前校验。 */
        ...
        res.id  = id;
        r = perm_check(&res, VFS_ACCESS_WRITE, req->path,
                   caller_subject, NULL);
        if (r < 0) { resp->ret = r; goto out; }
...
static void do_open(int token, int msg_len, u64 caller_subject)
{
     ...
     * it on every operation so a perm_revoke takes effect immediately. */
        ...
        res.id  = fid;
        u32 granted = 0;
        r = perm_check(&res, req->access, req->path,
                   caller_subject, &granted);
        if (r < 0) { resp->ret = r; goto out; }
     ...
     * never mutates the file. */
        ...
        vfs_handle_t htok;
        vfs_handle_ent_t *h = handle_alloc(vi, fid, granted, req->flags,
                                       caller_subject, &res, &htok);
        if (!h) {
                resp->ret = ERR_NOMEM;
                goto out;
        }
...
static void do_read(int token, int msg_len, u64 caller_subject)
{
     ...
     * reject a handle token leaked to another process. */
        ...
        int r = perm_check(&h->resource, VFS_ACCESS_READ,
                       "", h->subject_id, NULL);
        if (r < 0) { resp->ret = r; goto out; }
...
static void do_write(int token, int msg_len, u64 caller_subject)
{
     ...
     * caller must also be the handle's original opener. */
        ...
        int r = perm_check(&h->resource, VFS_ACCESS_WRITE,
                       "", h->subject_id, NULL);
        if (r < 0) { resp->ret = r; goto out; }
...
static void do_enum_begin(int token, int msg_len, u64 caller_subject)
{
     ...
     * batches takes effect immediately. */
        ...
        res.vol = s_vols[vi].uuid;
        res.id  = id;
        r = perm_check(&res, VFS_ACCESS_READ, req->path,
                   caller_subject, NULL);
        if (r < 0) { resp->ret = r; goto out; }
...
static void do_enum_next(int token, int msg_len, u64 caller_subject)
{
     ...
     * takes effect immediately. */
        ...
        int r = perm_check(&e->resource, VFS_ACCESS_READ,
                       "", e->subject_id, NULL);
        if (r < 0) { resp->ret = r; goto out; }
...
static void do_create_bookmark(int token, int msg_len, u64 subject_id)
{
     ...
     * grant can never mint a WRITE-carrying bookmark). */
        ...
        u32 tok;
        vfs_bookmark_ent_t *b = bookmark_alloc(vi, id, granted, subject_id,
                                           (u64)get_time(),
...
static void do_move(int token, int msg_len, u64 caller_subject)
{
     ...
     * 之前校验。 */
        ...
        res.id  = sid;
        r = perm_check(&res, VFS_ACCESS_WRITE, req->src,
                   caller_subject, NULL);
        if (r < 0) { resp->ret = r; goto out; }
        res.vol = s_vols[dvi].uuid;
        res.id  = did;
        r = perm_check(&res, VFS_ACCESS_WRITE, req->dst_dir,
                   caller_subject, NULL);
        if (r < 0) { resp->ret = r; goto out; }
                               ...
                               u64 caller_subject)
{
        ...
        case VFS_OP_CREATE_DIR:     do_create_dir(token, msg_len,
                                                                                            caller_subject); break;
        case VFS_OP_DELETE_ITEM:    do_delete(token, msg_len,
                                                                                    caller_subject); break;
        case VFS_OP_OPEN_ITEM:      do_open(token, msg_len, caller_subject); break;
        case VFS_OP_READ:           do_read(token, msg_len, caller_subject); break;
        case VFS_OP_WRITE:          do_write(token, msg_len, caller_subject); break;
        case VFS_OP_CLOSE:          do_close(token, msg_len); break;
        case VFS_OP_ENUM_BEGIN:     do_enum_begin(token, msg_len,
                                                                                            caller_subject); break;
        case VFS_OP_ENUM_NEXT:      do_enum_next(token, msg_len,
                                             caller_subject); break;
        case VFS_OP_MOUNT:          do_mount(token, msg_len); break;
        case VFS_OP_UNMOUNT:        do_unmount(token, msg_len); break;
        case VFS_OP_STAT_VOLUME:    do_stat_volume(token, msg_len); break;
        case VFS_OP_CREATE_BOOKMARK:  do_create_bookmark(token, msg_len,
                                                     caller_subject); break;
        case VFS_OP_RESOLVE_BOOKMARK: do_resolve_bookmark(token, msg_len,
                                                                                                            caller_subject); break;
        case VFS_OP_REVOKE_BOOKMARK:  do_revoke_bookmark(token, msg_len); break;
        case VFS_OP_MOVE:             do_move(token, msg_len,
                                                                                    caller_subject); break;
        case VFS_OP_WHOAMI:           do_whoami(token, msg_len, caller_subject);
                                                                    break;
        default: {
                i32 *resp = (i32 *)s_resp;
                *resp = ERR_INVAL;

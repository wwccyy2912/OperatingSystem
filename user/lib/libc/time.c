...
time_t time(time_t *timer)
{
        ...
        int leap_days = (years_since + 1) / 4 - (years_since + 69) / 100 +
                        (years_since + 369) / 400;
...
#define EMIT2(v) do { \
        ...
        while (*fmt && pos < max) {
                ...
                case 'm': EMIT2(t->tm_mon + 1); break;
                case 'd': EMIT2(t->tm_mday); break;
                case 'H': EMIT2(t->tm_hour); break;
                case 'M': EMIT2(t->tm_min); break;
                case 'S': EMIT2(t->tm_sec); break;
                case 'j': EMIT2((t->tm_yday / 100) % 10);
                          EMIT2(t->tm_yday % 100); break;
                case 'w': EMIT('0' + t->tm_wday); break;
                case 'a': for (int i = 0; i < 3; i++) EMIT(s_wday[t->tm_wday][i]); break;
                case 'A': {
                            const char *p = s_wday[t->tm_wday];
                            while (*p) EMIT(*p++);
                            break;
                        }
                case 'b': for (int i = 0; i < 3; i++) EMIT(s_mon[t->tm_mon][i]); break;
                case 'B': {

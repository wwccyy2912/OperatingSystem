...
static int heap_grow(size_t need)
{
    ...
    void *addr = map_memory(cap, (int)s_next_virt, (int)chunk,
...
void *malloc(size_t size)
{
    ...
    /* Total block size = header + aligned payload, rounded up to minimum */
    ...
    /* Retry loop: grows the heap inside the critical section, then
     * searches again.  (No recursive malloc() call — the kernel mutex
     * is non-recursive.) */
    for (;;) {
        /* First-fit search */
        block_t **pp = &s_free_list;
        while (*pp) {
            ...
            if (bsz >= asize) {
                ...
                if (remainder >= MALLOC_MIN_SIZE) {
                    /* Split: carve 'asize' from the front of this block */
        ...
        /* Out of memory: grow the heap and retry once */
...
void free(void *ptr)
{
    ...
    /* Insert at head of free list (singly-linked, sorted by address) */
    b->next = s_free_list;
    s_free_list = b;

    /* Coalesce with the immediately following block if it is free */
    coalesce_after(b);
...
void *realloc(void *ptr, size_t size)
{
    ...
    /* Allocate a new block, copy data, free old */

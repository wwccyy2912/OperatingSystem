...
static void idt_set_entry(idt_entry_t *entry, u64 handler,
                                                    u8 type_attr, u8 ist)
{
...
ISR_STUB_NOERR(0)   ISR_STUB_NOERR(1)   ISR_STUB_NOERR(2)   ISR_STUB_NOERR(3)
ISR_STUB_NOERR(4)   ISR_STUB_NOERR(5)   ISR_STUB_NOERR(6)   ISR_STUB_NOERR(7)
ISR_STUB_NOERR(9)
ISR_STUB_NOERR(15)  ISR_STUB_NOERR(16)
ISR_STUB_NOERR(18)  ISR_STUB_NOERR(19)  ISR_STUB_NOERR(20)
ISR_STUB_NOERR(22)  ISR_STUB_NOERR(23)  ISR_STUB_NOERR(24)  ISR_STUB_NOERR(25)
ISR_STUB_NOERR(26)  ISR_STUB_NOERR(27)  ISR_STUB_NOERR(28)
ISR_STUB_NOERR(31)

/* Exceptions WITH error codes (CPU pushes error code onto stack) */
ISR_STUB_ERR(8)
ISR_STUB_ERR(10)  ISR_STUB_ERR(11)  ISR_STUB_ERR(12)  ISR_STUB_ERR(13)
ISR_STUB_ERR(14)
ISR_STUB_ERR(17)
ISR_STUB_ERR(21)
ISR_STUB_ERR(29)  ISR_STUB_ERR(30)
...
void isr_handler(interrupt_frame_t *frame)
{
        ...
        if (vector < 32) {
                const char *name = (vector < 32 && s_exception_names[vector])
                           ? s_exception_names[vector] : "Unknown";

                serial_printf("\n====== CPU EXCEPTION ======\n");
                serial_printf("  %s (vector=%u, error=0x%x)\n", name,
                                            (u32)vector, (u32)err_code);
                serial_printf("  RIP  = %p\n", frame->rip);
             ...
             */
                        ...
                        if (th) {
                                ...
                                const char *state_names[] = {
                                        "READY", "RUNNING", "BLOCKED", "ZOMBIE", "FINISHED"
                                };
                                int sidx = (th->state >= 0 && th->state <= 4) ? th->state : 0;
                                serial_printf("  Thread: TID=%d PID=%d state=%s\n",
                                        th->tid, th->pid, state_names[sidx]);
                                serial_printf("  Kernel RSP=%p kstack=[%p-%p]\n",
...
void idt_register_handler(u8 vector, void (*handler)(void), u8 dpl)
{
        ...
        u8 type_attr = (dpl == 3) ? GATE_INTERRUPT_USER
                               : GATE_INTERRUPT_KERNEL;

        idt_set_entry(&idt[vector], (u64)isr_stub_table[vector],
                                    type_attr, 0);
}

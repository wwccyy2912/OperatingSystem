...
void kernel_main(u64 mboot_info_phys, u64 kernel_phys_start)
{
    ...
    u64 entry_addr = 0;
    {
        ...
        err = elf_boot_load(init_proc->addr_space, init_data, init_size,
                            &entry_addr);
        if (err != OK) {
    ...
    tid_t init_tid = thread_create_user(entry_addr, 0,
                                        init_proc->addr_space, 10);
    if (init_tid < 0) {
    ...
    {
        error_t err = vmm_map(init_proc->addr_space, boot_stack_virt, user_stack_phys,
    ...
    io_outb(0x43, 0x30);  /* Channel 0, lobyte/hibyte, mode 0 (interrupt on terminal count) */

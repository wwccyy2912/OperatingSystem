...
static void shell_putc(char c)
{
    ...
    ipc_call(s_term_port, (const void *)req, 9,
             (void *)resp, &resp_len);
}
...
static void shell_write(const char *s)
{
    ...
    while (*s != '\0') {
        ...
        req[1] = (u32)n;
        int resp_len = (int)sizeof(resp);
        ipc_call(s_term_port, (const void *)req, 8 + n,
                 (void *)resp, &resp_len);
        s += n;
    }
}
...
static int read_line(char *buf, int maxlen)
{
    ...
    for (;;) {
        ...
        req[1] = KBD_CHUNK;
        int resp_len = (int)sizeof(resp);
        int ret = ipc_call(s_kbd_port, (const void *)req, 8,
                           (void *)resp, &resp_len);
        if (ret < 0)
            return -1;
        i32 n = (i32)resp[0];
...
static void shell_loop(void)
{
    ...
    shell_write("\n");
    shell_write("  +-----------------------------------------+\n");
    shell_write("  |   OpSys Microkernel Shell v0.1          |\n");
    shell_write("  |   Type 'help' for available commands    |\n");
    shell_write("  +-----------------------------------------+\n");
    shell_write("\n");
...
static int cmd_spawn(int argc, char *argv[])
{
    (void)argc;
    (void)argv;
    static char blob_buf[65536];   /* must hold hello.elf (~30 KB) */
...
static int cmd_meminfo(int argc, char *argv[])
{
    ...
    shell_printf("Free memory: %d pages (%d KB, %d MB)\n",
...
static int cmd_clear(int argc, char *argv[])
{
    ...
    if (s_term_port >= 0) {
        ...
        req[1] = 0;
        int resp_len = (int)sizeof(resp);
        ipc_call(s_term_port, (const void *)req, 8,
                 (void *)resp, &resp_len);
    }
    return 0;
}
...
static int cmd_ps(int argc, char *argv[])
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        switch (p->state) {
        case 0:  state_name = "CREATED";  break;
        case 1:  state_name = "READY";    break;
        case 2:  state_name = "RUNNING";  break;
        case 3:  state_name = "ZOMBIE";   break;
        case 4:  state_name = "FINISHED"; break;
...
static int cmd_vfs_stat(int argc, char *argv[])
{
    ...
    for (int v = 0; v < count; v++) {
        ...
        shell_printf("%s: %d KB total, %d KB used, %s\n",
                     url, (int)(total / 1024), (int)(used / 1024),
...
static int cmd_vfs_write(int argc, char *argv[])
{
    ...
    vfs_handle_t h;
    int r = fs_open_item(url, VFS_OPEN_CREATE | VFS_OPEN_TRUNCATE,
                         VFS_ACCESS_WRITE, &h);
    if (r < 0) {
 ...
 *   perm_revoke [app_hash]   → grants dropped (or bm_revoke)
 *   bm_resolve               → -105 again (撤销后 → -EACCES)
 * ==================================================================== */

/* Fixed test app identity: the shell plays the sandboxed app role. */
#define SHELL_APP_HASH  0x5E11E5u
...
static u32 bm_access(const char *s)
{
    ...
    if (!s || *s == '\0')
        s = "r";
    for (; *s; s++) {
        if (*s == 'r' || *s == 'R') a |= VFS_ACCESS_READ;
        if (*s == 'w' || *s == 'W') a |= VFS_ACCESS_WRITE;
        if (*s == 'x' || *s == 'X') a |= VFS_ACCESS_EXEC;
    }
    return a;
}
...
static int cmd_bm_create(int argc, char *argv[])
{
    ...
    u32 len = 0;
    int r = fs_create_bookmark(url, access, SHELL_APP_HASH, 0,
                               s_bm_blob, &len);
    if (r < 0) {
...
static int cmd_bm_resolve(int argc, char *argv[])
{
    ...
    shell_printf("bm_resolve: handle %d, item '%s' (id %d), access %d\n",
                 (int)h, item.name, (int)item.item_id, (int)access);
    fs_close(h);
    return 0;
}
...
static int cmd_perm_answer(int argc, char *argv[])
{
    ...
    shell_printf("perm_answer: query %d -> %s (%d)\n",
                 (int)req.query_id, req.allow ? "ALLOWED" : "DENIED", resp.ret);
    return 0;
}

/* perm_query [query_id] — fetch a pending Powerbox query (0 = first
 * pending, FIFO).  Prints id/app/url/access/state so the test flow can
 * discover the query id that bm_create left pending. */
static int cmd_perm_query(int argc, char *argv[])
{
    int port = perm_port();
    if (port < 0) {
    ...
    shell_printf("perm_query: id=%d app=0x%x url=%s access=%d state=%d\n",
                 (int)resp.query_id, resp.app_id_hash, resp.url,
                 (int)resp.access, (int)resp.state);
    return 0;
}

/* perm_revoke [app_id_hash] — drop grants for an app (0 = all apps,
 * all resources).  Default = the shell's test hash. */
static int cmd_perm_revoke(int argc, char *argv[])
{
    int port = perm_port();
    if (port < 0) {
    ...
    u32 app = (argc >= 2) ? (u32)atoi(argv[1]) : SHELL_APP_HASH;
...
static int cmd_move(int argc, char *argv[])
{
    ...
    vfs_item_info_t item;
    int r = fs_move_item(argv[1], argv[2], (argc >= 4) ? argv[3] : "",
                         &item);
    if (r < 0) {
...
static void shell_main(void *arg)
{
    ...
    shell_register_command("bm_create", "Powerbox-gated bookmark: bm_create <url> [r|w|rw]", cmd_bm_create);
    ...
    shell_register_command("perm_answer", "Answer a Powerbox query: perm_answer <id> y|n", cmd_perm_answer);
    shell_register_command("perm_query", "Show pending Powerbox query: perm_query [id]", cmd_perm_query);
    shell_register_command("perm_revoke", "Drop grants: perm_revoke [app_id_hash]", cmd_perm_revoke);

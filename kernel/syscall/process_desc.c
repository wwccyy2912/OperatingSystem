 ...
 */
static error_t map_segment(addr_space_t *as, const proc_seg_desc_t *seg,
                           const u8 *blob)
{
        u64 page_vaddr = seg->vaddr & ~(PAGE_SIZE - 1);
        u64 page_end   = (seg->vaddr + seg->memsz + PAGE_SIZE - 1) &
                     ~(PAGE_SIZE - 1);
...
i64 sc_sys_process_create(u64 a1, u64 a2, u64 a3, u64 a4, u64 a5)
{
     ...
     * the whole range is mapped before dereferencing it. */
        const proc_seg_desc_t *segs =
                (const proc_seg_desc_t *)((const u8 *)USER_PTR(a2) +
                                                                    sizeof(proc_image_desc_t));
        if (!validate_user_ptr(a2 + sizeof(proc_image_desc_t),
                           desc.seg_count * sizeof(proc_seg_desc_t), false))
                return (i64)ERR_FAULT;
        ...
        for (u64 si = 0; si < desc.seg_count; si++) {
                ...
                if (seg->memsz < seg->filesz) {
                        serial_printf("proc: seg %d memsz 0x%x < filesz 0x%x\n",
                                                    (int)si, (u32)seg->memsz, (u32)seg->filesz);
                        return (i64)ERR_INVAL;
                }
                /* prot must be a subset of PROT_NONE|PROT_READ|PROT_WRITE|PROT_EXEC */
                if ((seg->prot & ~(u64)(PROT_NONE | PROT_READ | PROT_WRITE |
                                                                PROT_EXEC)) != 0) {
                ...
                if (seg->memsz > USER_PTR_MAX ||
                        seg->vaddr > USER_PTR_MAX - seg->memsz) {
                        serial_printf("proc: seg %d vaddr out of user space\n", (int)si);
                        return (i64)ERR_INVAL;
                }
                /* File data [src_offset, src_offset+filesz) must lie in blob. */
                if (seg->filesz > blob_size ||
                        seg->src_offset > blob_size - seg->filesz) {
                        serial_printf("proc: seg %d blob range out of bounds\n", (int)si);
                        return (i64)ERR_INVAL;
                }
        }

        serial_printf("proc: CREATE name=%s entry=0x%x segs=%d\n",
     ...
     * destroys the address space itself. */
        ...
        if (proc->cap_table) {
                ...
                for (u64 bi = 0;
                     bi < sizeof(s_svc_blobs) / sizeof(s_svc_blobs[0]); bi++) {
                        const void *blob_data = NULL;
                        u64 blob_sz = 0;
                        if (blob_get(s_svc_blobs[bi], &blob_data, &blob_sz) != OK)
                                continue;
                        if (blob_size == blob_sz &&
                            memcmp(blob, blob_data, blob_sz) == 0) {
                                cap_t h = CAP_NULL;
                                cap_create_atom(proc->cap_table, proc->subject_id,
                                                ATOM_SERVICE_MANAGE, RIGHT_ALL,
                                                0, 0, 0, &h);
                                break;
                        }
                }

...
typedef u32 vfs_handle_t;   /* server-side handle-table index, 0 invalid */
...
/* Open flags — Phase 0 implements the first three */
 ...
 *
 * The bookmark blob is an OPAQUE token handed to the client; the
 * vfs_server holds the authoritative record table (design §5.2:
 * "Phase 2 的书签有效性以 vfs_server 服务端记录表 + 随机 token 为准").
 * The blob layout mirrors vfs_bookmark_t below; mac[16] stays zero
 * until Phase 3 adds a real HMAC.
 * ==================================================================== */
...
typedef struct {
    ...
    u32            version;           /* = 1 */
    u32            payload_len;       /* bytes after reserved */
    u32            reserved;          /* server-side token */
    /* --- payload (version 1) --- */
    ...
    u32            app_id_hash;       /* requesting app id hash */
...
typedef struct {
    ...
    u32  app_id_hash;             /* client-supplied app identity */
...
enum {
    ...
    DRV_OP_WRITE      = 4,   /* item_id + offset + data → bytes;

 ...
 * Uses a 64 KB static staging buffer (the manager's spawn idiom proves
 * it holds the largest service ELF), then copies each payload into an
 * exact-size heap allocation owned by the item.
 */
static int mem_sys_load(void)
{
    static char blob_buf[65536];

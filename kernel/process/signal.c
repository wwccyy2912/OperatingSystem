 ...
 */
static bool signal_check_common(process_t *proc, u64 *gprs,
                                u64 *rip, u64 *rsp, u64 *rflags)
{
    ...
    for (int signum = 1; signum < NSIG; signum++) {
        ...
        if (base < 0x1000 || proc->sig_restorer == 0 ||
            !vmm_validate_user_range(proc->addr_space, base - 8,
                                     SIGFRAME_TOTAL, true)) {
...
bool signal_check_syscall(u64 *frame)
{
    ...
    return signal_check_common(proc, frame,
                               &frame[SF_RIP_IDX],
                               &frame[SF_RSP_IDX],
                               &frame[SF_RFLAGS_IDX]);
}
...
i64 signal_restore(u64 frame_ptr)
{
    ...
    if (!proc || !proc->addr_space)
        return (i64)ERR_FAULT;
    if (!vmm_validate_user_range(proc->addr_space, frame_ptr,
                                 sizeof(sigframe_t), false))
        return (i64)ERR_FAULT;

 ...
 * registered.
 */
int shell_register_command(const char *name, const char *help,
                           cmd_func_t func) {
...
static int read_line(char *buf, int maxlen) {
  ...
  for (;;) {
    ...
    int resp_len = (int)sizeof(resp);
    int ret =
        ipc_call(s_kbd_port, (const void *)req, 8, (void *)resp, &resp_len);
    if (ret < 0)
      return -1;
    i32 n = (i32)resp[0];
    if (n < 0)
...
static int cmd_vfs_stat(int argc, char *argv[]) {
  ...
  for (int v = 0; v < count; v++) {
    ...
    shell_printf("%s: %d KB total, %d KB used, %s\n", url, (int)(total / 1024),
                 (int)(used / 1024), ro ? "read-only" : "read-write");
  }
  return 0;
}
...
static int cmd_bm_resolve(int argc, char *argv[]) {
  ...
  shell_printf("bm_resolve: handle %d, item '%s' (id %d), access %d\n", (int)h,
               item.name, (int)item.item_id, (int)access);
  fs_close(h);
  return 0;
}
...
static int cmd_perm_answer(int argc, char *argv[]) {
  ...
  shell_printf("perm_answer: query %d -> %s (%d)\n", (int)req.query_id,
               req.allow ? "ALLOWED" : "DENIED", resp.ret);
  return 0;
}
...
static int cmd_perm_query(int argc, char *argv[]) {
  ...
  shell_printf("perm_query: query %d: %s (PID %d) requests %s (%s) - %s "
               "[state %d]\n",
               (int)resp.query_id, resp.name, (int)resp.pid, resp.url,
               perm_access_str(resp.access), resp.label, (int)resp.state);
  return 0;
}
...
static int cmd_perm_revoke(int argc, char *argv[]) {
  ...
  u64 subject =
      (argc >= 2) ? (u64)strtoull(argv[1], NULL, 10) : get_subject();
...
static int cmd_move(int argc, char *argv[]) {
  ...
  shell_printf("move: '%s' -> item %d (size %d)\n", item.name,
               (int)item.item_id, (int)item.size);
  return 0;
}
...
static void shell_main(void *arg) {
  ...
  shell_register_command("sleep", "Sleep for N ticks: sleep <ticks>",
                         cmd_sleep);
  shell_register_command("threads", "Spawn a test worker thread", cmd_threads);
  shell_register_command("mutex", "Mutex demo: N threads on a counter",
                         cmd_mutex);
  ...
  shell_register_command("panic", "Trigger a kernel panic (TEMP test)",
                         cmd_panic);
  shell_register_command("kill", "Send a signal: kill <pid> [signum]",
                         cmd_kill);
  shell_register_command("ps", "List running processes", cmd_ps);
  shell_register_command("vfs_ls", "List a VFS directory: vfs_ls <url>",
                         cmd_vfs_ls);
  shell_register_command("vfs_cat", "Show a VFS file: vfs_cat <url>",
                         cmd_vfs_cat);
  shell_register_command("vfs_stat", "VFS volume stats: vfs_stat [url]",
                         cmd_vfs_stat);
  shell_register_command(
      "vfs_write", "Write a VFS file: vfs_write <url> <text>", cmd_vfs_write);
  shell_register_command(
      "vfs_fill", "Fill a volume until ENOSPC: vfs_fill <url>", cmd_vfs_fill);
  shell_register_command("vfs_mkdir", "Create a VFS dir: vfs_mkdir <url>",
                         cmd_vfs_mkdir);
  shell_register_command("vfs_rm", "Delete a VFS item: vfs_rm <url>",
                         cmd_vfs_rm);
  shell_register_command("bm_create",
                         "Powerbox-gated bookmark: bm_create <url> [r|w|rw]",
                         cmd_bm_create);
  shell_register_command("bm_resolve", "Resolve cached bookmark to a handle",
                         cmd_bm_resolve);
  shell_register_command("bm_revoke", "Drop the cached bookmark server-side",
                         cmd_bm_revoke);
  shell_register_command("perm_answer",
                         "Answer a Powerbox query: perm_answer <id> y|n",
                         cmd_perm_answer);
  shell_register_command("perm_query",
                         "Show pending Powerbox query: perm_query [id]",
                         cmd_perm_query);
  shell_register_command(
      "perm_revoke", "Drop grants: perm_revoke [subject_id]", cmd_perm_revoke);
  shell_register_command("move", "Move/rename: move <src> <dst-dir> [new-name]",
                         cmd_move);
  shell_register_command(
      "pkg", "pkg-manager: pkg <install|list|run|remove>", cmd_pkg);

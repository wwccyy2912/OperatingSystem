...
| **v0.4** | 图形 + 窗口管理器（显示桌面） | ✅ 已完成（2026-08-22）：`wm` 窗口管理器服务（窗口注册表 + 合成器经 term 渲染 + 键盘焦点路由）+ `libwm` 客户端库 + `wm_demo` 桌面演示；`scripts/verify_wm.py` 8/8 通过 |
| **v0.5** | 用户账户与退出保护 + 环境变量 + 命令策略（实际落地内容与最初的"音频+网络"设想不同） | ✅ 已完成（2026-08-23） |
| **v0.6** | 架构优化系列（性能/安全/UX/去冗余，v0.6.1–v0.6.9） | ✅ 已完成（2026-08-25） |
| **v0.7** | SYSCALL 快速路径、DMA 超时重试、金丝雀自检、终端滚动历史（v0.7.1 收尾） | ✅ 已完成（2026-08-26） |
| **v0.8-dev** | 像素 GUI 合成器 + PS/2 鼠标 + 网络协议栈（PCnet/ARP/IPv4/ICMP/UDP/TCP） + UTF-8/CJK/IME | 🚧 进行中 |
| **v1.0** | 稳定版，支持多核、性能优化、文档齐全 | ⏳ 规划 |

> **实现进展提示**：上表中 v0.5 之后的实际交付与最初规划存在差异（例如网络在 v0.8-dev 落地、音频尚未开始）。
> 逐提交的实际进展见 [CHANGELOG.md](CHANGELOG.md)；当前功能全貌见 [../README.md](../README.md)；系统调用规模已从最初设想的"约 12 个"增长到 69 个，逐项清单见 [syscall_reference.md](syscall_reference.md)。
...
- 全局函数：`模块名_动词_名词`（如 `pmm_alloc_page`）；静态函数：`_前缀`。
  > **已更新（2026-09-01）**：项目实际采用的是 **PascalCase** 函数名（如 `PmmAllocPage`、`IpcCall`），
  > 由提交 `4877fcd`/`aa2b97b` 对 `kernel/` 与 `user/` 全量统一；libc 标准函数名保持原样。
  > 现行规范见 [developer_guide.md](developer_guide.md) §代码风格。

...
static inline u64 pdp_index(u64 virt)  { return (virt >> 30) & 0x1FF; }
...
static bool page_is_mapped(addr_space_t *as, u64 vaddr, bool need_write)
{
        ...
        if (!(pml4[idx] & PTE_PRESENT)) return false;

        u64 *pdp = (u64 *)phys_to_virt(pml4[idx] & ~0xFFFULL);
        idx = pdp_index(vaddr);
        if (!(pdp[idx] & PTE_PRESENT)) return false;
        ...
        u64 *pd = (u64 *)phys_to_virt(pdp[idx] & ~0xFFFULL);
        idx = pd_index(vaddr);
        if (!(pd[idx] & PTE_PRESENT)) return false;
        ...
        u64 *pt = (u64 *)phys_to_virt(pd[idx] & ~0xFFFULL);
        idx = pt_index(vaddr);
        if (!(pt[idx] & PTE_PRESENT)) return false;
...
addr_space_t *vmm_create_addr_space(void) {
     ...
     * stack at stack_base + tid*PAGE_SIZE. */
        ...
        serial_printf("VMM: Created addr space, PML4 at 0x%x, stack_base=0x%x\n",
                                    (u32)pml4_phys, (u32)as->stack_base);
        return as;
}
...
void vmm_destroy_addr_space(addr_space_t *as) {
        ...
        for (int pml4_i = 0; pml4_i < 256; pml4_i++) {
                ...
                for (int pdp_i = 0; pdp_i < 512; pdp_i++) {
                        if (!(pdp[pdp_i] & PTE_PRESENT)) continue;
...
error_t vmm_map(addr_space_t *as, u64 virt, u64 phys, u64 flags) {
     ...
     * slipped past syscall-layer checks. */
        if (virt >= USER_PTR_MAX) return ERR_INVAL;

        u64 *pte = walk_page_table(as, virt, true);
        if (!pte) return ERR_NOMEM;
...
error_t vmm_unmap(addr_space_t *as, u64 virt) {
     ...
     * page tables from a user-supplied VA. */
        ...
        /* PML4 -> PDP */
        idx = pml4_index(virt);
        if (!(pml4[idx] & PTE_PRESENT)) return ERR_NOENT;
        u64 *pdp = (u64 *)phys_to_virt(pml4[idx] & ~0xFFFULL);

        /* PDP -> PD */
        idx = pdp_index(virt);
        if (!(pdp[idx] & PTE_PRESENT)) return ERR_NOENT;
        if (pdp[idx] & PTE_HUGE) {
                pdp[idx] = 0;
        ...
        /* PD -> PT */
        idx = pd_index(virt);
        if (!(pd[idx] & PTE_PRESENT)) return ERR_NOENT;
        if (pd[idx] & PTE_HUGE) {
                pd[idx] = 0;
                __asm__ volatile("invlpg (%0)" : : "r"(virt) : "memory");
                return OK;
...
error_t vmm_map_range(addr_space_t *as, u64 virt, u64 phys,
        ...
        for (u64 i = 0; i < page_count; i++) {
                error_t err = vmm_map(as, virt + i * PAGE_SIZE,

...
static void ipc_recv_worker(void *arg) {
  ...
  g_ipc_recv_ret =
      ipc_recv(g_ipc_recv_port, g_ipc_recv_buf, &g_ipc_recv_len, &tok);
  thread_exit(0);
}
...
static void bench_yield_100k(void) {
  ...
  /* user printf lacks %llu: print 64-bit cycles as hi/lo 32-bit halves */
  unsigned long long dc = c1 - c0;
  printf("(%d yields, %d ticks, cyc_hi=%u cyc_lo=%u, other_done=%d) ", 1000,
         elapsed, (unsigned)(dc >> 32), (unsigned)(dc & 0xFFFFFFFFu),
...
static void test_vspace_smoke(void) {
  ...
  void *mapped =
      map_memory(mem_cap, (int)(long)base, 0x4000, PROT_READ | PROT_WRITE);
...
static void test_stress_threads_1000(void) {
   ...
   * page-table pages.  128 pages reserves ~512KB which is ample. */
  {
    ...
    if (max_threads > max_concurrent) {
      printf("(low-memory: capping to %d threads, %d reserve) ", max_concurrent,
             reserve);
  ...
  while (i < max_threads) {
    int batch_count =
        (max_threads - i < BATCH_SIZE) ? (max_threads - i) : BATCH_SIZE;
    int batch_created = 0;
  ...
  ASSERT(g_stress_counter == max_threads,
         "shared counter != expected count (lost increments?)");
...
static int p1_create_bookmark(int vfs_port, u32 access,
...
static void test_p1_owner_auto_allow(void) {
  ...
  P1_ASSERT(g_p1_res.vol.hi != 0 || g_p1_res.vol.lo != 0,
            "captured resource empty");
  P1_PASS();
}
...
static void test_p1_role_set_hot_reload(void) {
  ...
  P1_ASSERT(p1_create_bookmark(vfs_port, VFS_ACCESS_WRITE, &cr) ==
                VFS_ERR_ACCESS,
...
static void test_p1_chain_deny_no_prompt(void) {
  ...
  vfs_resp_create_bookmark_t cr;
  P1_ASSERT(p1_create_bookmark(vfs_port, VFS_ACCESS_READ, &cr) ==
                VFS_ERR_ACCESS,
  ...
  P1_ASSERT(ipc_call(perm_port, &q, (int)sizeof(q), &qr, &rlen) == 0,
...
static void test_p1_powerbox_allow(void) {
  ...
  vfs_resp_create_bookmark_t cr;
  P1_ASSERT(p1_create_bookmark(vfs_port, VFS_ACCESS_EXEC, &cr) ==
                VFS_ERR_ACCESS,
  ...
  rlen = (int)sizeof(ar);
  P1_ASSERT(ipc_call(perm_port, &a, (int)sizeof(a), &ar, &rlen) == 0 &&
                ar.ret == 0,
            "ANSWER failed");
...
static void test_p1_revoke(void) {
  ...
  P1_ASSERT(ipc_call(perm_port, &r, (int)sizeof(r), &rr, &rlen) == 0,
            "REVOKE transport failed");
...
static void test_p1_grant_beats_role(void) {
  ...
  P1_ASSERT(ipc_call(perm_port, &g, (int)sizeof(g), &gr, &rlen) == 0 &&
                gr.ret == 0,
            "GRANT failed");
...
static void test_p1_dump(void) {
  ...
  P1_ASSERT(ipc_call(perm_port, &d, (int)sizeof(d), &dr, &rlen) == 0 &&
                dr.ret == 0,
            "DUMP failed");
...
static void test_p1_grant_to_subject(void) {
  ...
  P1_ASSERT(cap_grant_to_subject(0x7FFFFFFFULL, ATOM_DATA_DOCS_READ, RIGHT_ALL,
                                 0, 0) == ERR_NOENT,
...
static int p2v_open(int vfs_port, const char *path, u32 flags, u32 access,
                    vfs_resp_open_t *resp) {
  vfs_req_open_t req;
  memset(&req, 0, sizeof(req));
...
static void test_p2v_open_gate(void) {
  ...
  int ret = p2v_open(vfs_port, "Users/p2vfs.bin", VFS_OPEN_CREATE,
                     VFS_ACCESS_WRITE, r1);
  printf("(unauth=%d) ", ret);
  ...
  int rlen = (int)sizeof(*rr);
  P2V_ASSERT(ipc_call(vfs_port, &rd, (int)sizeof(rd), rr, &rlen) == 0,
             "READ transport failed");
...
static void test_p2v_capability_trim(void) {
  ...
  int rlen = (int)sizeof(*gr);
  P2V_ASSERT(ipc_call(perm_port, g, (int)sizeof(*g), gr, &rlen) == 0 &&
                 gr->ret == 0,
             "GRANT(READ) failed");
  ...
  int ret =
      p2v_open(vfs_port, P1_ITEM_PATH, 0, VFS_ACCESS_READ | VFS_ACCESS_EXEC, r);
  printf("(open=%d) ", ret);
...
static void test_p2v_p3p4_ifaces(void) {
  ...
  P2V_ASSERT(ipc_call(perm_port, c, (int)sizeof(*c), cr, &rlen) == 0 &&
                 cr->ret == 0,
             "CONTEXT failed");
  ...
  rlen = (int)sizeof(*fr);
  P2V_ASSERT(ipc_call(perm_port, f, (int)sizeof(*f), fr, &rlen) == 0 &&
                 fr->ret == 0,
             "FREQ query failed");
  ...
  rlen = (int)sizeof(*pr1);
  P2V_ASSERT(ipc_call(perm_port, ps, (int)sizeof(*ps), pr1, &rlen) == 0 &&
                 pr1->ret == 0 && pr1->size > 0,
             "POLICY_SAVE failed");
  ...
  rlen = (int)sizeof(*lr);
  P2V_ASSERT(ipc_call(perm_port, pl, (int)sizeof(*pl), lr, &rlen) == 0 &&
                 lr->ret == 0,
             "POLICY_LOAD failed");

  perm_resp_policy_t *pr2 = &s_p_pol3;
  rlen = (int)sizeof(*pr2);
  P2V_ASSERT(ipc_call(perm_port, ps, (int)sizeof(*ps), pr2, &rlen) == 0 &&
                 pr2->ret == 0,
  ...
  rlen = (int)sizeof(*ar);
  P2V_ASSERT(ipc_call(perm_port, a, (int)sizeof(*a), ar, &rlen) == 0 &&
                 ar->ret == 0 && ar->count > 0,
             "AUDIT failed");
  int g = 0, d = 0;
...
static void test_p2v_ops_gate(void) {
  ...
  rlen = (int)sizeof(*gir);
  P2V_ASSERT(ipc_call(vfs_port, gi, (int)sizeof(*gi), gir, &rlen) == 0 &&
                 gir->ret == 0,
  ...
  P2V_ASSERT(ipc_call(vfs_port, eb2, (int)sizeof(*eb2), ebr2, &rlen) == 0 &&
                 ebr2->ret == 0 && ebr2->handle != 0,
  ...
  rlen = (int)sizeof(*rvr);
  P2V_ASSERT(ipc_call(perm_port, rv, (int)sizeof(*rv), rvr, &rlen) == 0 &&
                 rvr->ret == 0 && rvr->revoked >= 1,

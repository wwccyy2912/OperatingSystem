...
static void format_number(char *buf, int bufsize, int *pos,
                          unsigned long long val, int neg,
                          const struct fmt_spec *s)
{
        char numbuf[32];
        int numlen;
        ...
        /* Sign / prefix */
        char sign = 0;
        if (s->is_signed && neg) sign = '-';
        else if (s->is_signed && s->plus_sign) sign = '+';
        else if (s->is_signed && s->space_sign) sign = ' ';

        const char *prefix = "";
        int prefix_len = 0;
        if (s->alt_form && val != 0) {
                if (s->base == 16) { prefix = s->upper ? "0X" : "0x"; prefix_len = 2; }
                else if (s->base == 8) { prefix = "0"; prefix_len = 1; }
        }
        ...
        if (!s->left_align && pad > 0) {
                if (s->zero_pad && !s->has_precision) {
                        ...
                        if (sign) emit_char(buf, bufsize, pos, sign);
...
static int format_string(char *buf, int bufsize, const char *fmt, va_list args)
{
        ...
        while (*fmt) {
                ...
                /* Flags */
                for (;;) {
                        if      (*fmt == '0') { s.zero_pad = 1; fmt++; }
                        else if (*fmt == '-') { s.left_align = 1; fmt++; }
                        else if (*fmt == '+') { s.plus_sign = 1; fmt++; }
                        else if (*fmt == ' ') { s.space_sign = 1; fmt++; }
                        else if (*fmt == '#') { s.alt_form = 1; fmt++; }
                        else break;
                }
                ...
                if (*fmt >= '1' && *fmt <= '9') {
                        ...
                        if (s.width < 0) { s.left_align = 1; s.width = -s.width; }
                        fmt++;
                }
                ...
                while (*fmt == 'l' || *fmt == 'h' || *fmt == 'z' ||
                       *fmt == 'j' || *fmt == 't') {
                        if      (*fmt == 'l') { len_l++; fmt++; }
                        else if (*fmt == 'h') { len_h++; fmt++; }
                        else if (*fmt == 'z') { len_z = 1; fmt++; }
                        else if (*fmt == 'j') { len_j = 1; fmt++; }
                        else if (*fmt == 't') { len_t = 1; fmt++; }
                }
                ...
                case 'i': {
                        long long val;
                        if (len_l >= 2 || len_j) val = va_arg(args, long long);
                        else if (len_l == 1) val = va_arg(args, long);
                        else if (len_z || len_t) val = va_arg(args, long);
                        else val = va_arg(args, int);
                        if (len_h == 1) val = (short)val;
                        if (len_h >= 2) val = (signed char)val;
                        ...
                        s.base = 10; s.is_signed = 1;
                        format_number(buf, bufsize, &pos, uval, neg, &s);
                        break;
                }
                case 'u': {
                        unsigned long long val;
                        if (len_l >= 2 || len_j) val = va_arg(args, unsigned long long);
                        else if (len_l == 1 || len_z || len_t)
                                val = va_arg(args, unsigned long);
                        else val = va_arg(args, unsigned int);
                        if (len_h == 1) val = (unsigned short)val;
                        if (len_h >= 2) val = (unsigned char)val;
                        s.base = 10; s.is_signed = 0;
                ...
                case 's': {
                        ...
                        int slen = 0;
                        if (s.has_precision) {
                                while (str[slen] && slen < s.precision) slen++;
                        } else {
                                slen = (int)strlen(str);
                        }

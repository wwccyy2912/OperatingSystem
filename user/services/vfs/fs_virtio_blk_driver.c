...
#define VBDK_INODE_TABLE_SECTORS (VBDK_MAX_INODES * VBDK_INODE_SIZE / \
                                                                    VBDK_SECTOR_SIZE)                    /* 64 */
#define VBDK_INODE_TABLE_START  1                /* sector               */
#define VBDK_DATA_START         (VBDK_INODE_TABLE_START + \
                                 VBDK_INODE_TABLE_SECTORS)             /* 65 */
...
static i32 vbdk_inode_store(const vbdk_inode_t *in, u64 num)
{
        ...
        size_t off = (size_t)((num - 1) % VBDK_INODES_PER_SECTOR) *
                 VBDK_INODE_SIZE;
...
static i32 vbdk_lookup(vfs_item_id_t parent, const char *name,
                       vfs_item_id_t *out)
{
        if (!name)
                return ERR_INVAL;
...
static i32 vbdk_create_item(vfs_item_id_t parent, const char *name,
                                                        u32 type, vfs_item_id_t *out_id)
{
        if (!name || !name[0])
                return ERR_INVAL;
...
static i32 vbdk_create(vfs_item_id_t parent, const char *name, u32 type,
                       vfs_item_id_t *out_id)
{
        if (s_vol.read_only)
 ...
 */
static i32 vbdk_move(vfs_item_id_t id, vfs_item_id_t new_parent,
                     const char *name)
{
        if (s_vol.read_only)
                return VFS_ERR_READONLY;
...
static i32 vbdk_rw_write(u64 off, u32 len, const u8 *data,
                         const vbdk_inode_t *it)
{
        static u8 sbuf[VBDK_SECTOR_SIZE];
        u64 end = off + len;
        for (u64 s = off / VBDK_SECTOR_SIZE;
         s <= (end - 1) / VBDK_SECTOR_SIZE; s++) {
                u64 sect_off = s * VBDK_SECTOR_SIZE;
                u64 cstart = (off > sect_off) ? off : sect_off;
                u64 cend = (end < sect_off + VBDK_SECTOR_SIZE)
                       ? end : sect_off + VBDK_SECTOR_SIZE;
 ...
 * fits the extent. */
static i32 vbdk_rw_read(u64 off, u32 len, u8 *out,
                                                const vbdk_inode_t *it)
{
        static u8 sbuf[VBDK_SECTOR_SIZE];
        u64 end = off + len;
...
static i32 vbdk_zero_range(u64 from, u64 to, const vbdk_inode_t *it)
{
        ...
        if (to <= from)
                return 0;
        u64 end = to;
        for (u64 s = from / VBDK_SECTOR_SIZE;
         s <= (end - 1) / VBDK_SECTOR_SIZE; s++) {
                u64 sect_off = s * VBDK_SECTOR_SIZE;
                ...
                if (r < 0)
                        return r;
                memset(sbuf + (size_t)(cstart - sect_off), 0,
               (size_t)(cend - cstart));
                r = vbdk_sector_write((u64)it->extent_start + s, sbuf);
                if (r < 0)
                        return r;
...
static i32 vbdk_format(void)
{
     ...
     * index, geometry) across the RAM-table reset. */
        ...
        printf("fs_virtio_blk: formatted Disk (uuid=%x-%x-%x-%x)\n",
           (unsigned)(u32)(s_vol.uuid_hi >> 32), (unsigned)(u32)s_vol.uuid_hi,
           (unsigned)(u32)(s_vol.uuid_lo >> 32), (unsigned)(u32)s_vol.uuid_lo);
        return 0;
}
...
static i32 vbdk_load(void)
{
        ...
        if (sb.block_size != VBDK_SECTOR_SIZE ||
                sb.inode_table_start != VBDK_INODE_TABLE_START ||
                sb.inode_table_sectors != VBDK_INODE_TABLE_SECTORS ||
...
static void drv_handle(int token, drv_req_t *req)
{
     ...
     * the process stays alive without corrupting the namespace. */
        ...
        case DRV_OP_GETATTR:
                ...
                break;
        case DRV_OP_LOOKUP:
                resp->ret = vbdk_lookup(req->parent_id, req->payload.name,
                                                                &resp->u.item_id);
                break;
        case DRV_OP_READ:
                resp->ret = vbdk_read(req->item_id, req->offset, req->len,
                                                            resp->u.data);
                break;
        case DRV_OP_WRITE:
                resp->ret = vbdk_write(req->item_id, req->offset, req->len,
                               req->payload.data);
                break;
        case DRV_OP_CREATE_DIR:
                resp->ret = vbdk_create(req->parent_id, req->payload.name,
        ...
        case DRV_OP_ENUM:
                ...
                break;
        case DRV_OP_MOVE:
                resp->ret = vbdk_move(req->item_id, req->parent_id,
                                                            req->payload.name);
                break;
        case DRV_OP_STAT:
...
int main(void)
{
     ...
     * 0x1001) in the userspace PCI enumeration.  Index = disk arg. */
        ...
        for (int i = 0; i < count; i++) {
                ...
                if (pci_get_device(i, &dev) < 0)
                        continue;
                if (dev.vendor_id == VBDK_VIRTIO_VENDOR &&
                        dev.device_id == VBDK_VIRTIO_DEVICE) {
                        s_vol.disk = i;
     ...
     * both rights (every SYS_BLK_* requires it). */
        int cap = cap_create_obj(CAP_TYPE_PCI_DEV, RIGHT_READ | RIGHT_WRITE,
                             (unsigned long)s_vol.disk);
        if (cap < 0) {
     ...
     * the first sys_blk_* call, safe on a fresh boot. */
        ...
        if (r < 0 || info.sector_size != VBDK_SECTOR_SIZE ||
                info.sectors == 0) {
                printf("fs_virtio_blk: sys_blk_info failed (%d)\n", (int)r);
                vbdk_degrade("bad device geometry");
        }
        s_vol.nsectors = (info.sectors > VBDK_MAX_SECTORS)
                         ? VBDK_MAX_SECTORS : (u32)info.sectors;
        s_vol.total_bytes = (u64)(s_vol.nsectors - VBDK_DATA_START) *
                                                VBDK_SECTOR_SIZE;
        printf("fs_virtio_blk: device ready - %u sectors, %u bytes/sector\n",
           s_vol.nsectors, (unsigned)info.sector_size);
